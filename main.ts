function updatePlayerAnimation () {
    if (sprites.readDataNumber(playerSprite, "frame") == 0) {
        playerSprite.setImage(frames[14])
        sprites.setDataNumber(playerSprite, "frame", 1)
    } else {
        playerSprite.setImage(frames[13])
        sprites.setDataNumber(playerSprite, "frame", 0)
    }
    sprites.changeDataNumberBy(playerSprite, "y", sprites.readDataNumber(playerSprite, "vy"))
}
...
function jumpPlayer () {
    if (sprites.readDataNumber(playerSprite, "y") == 0) {
        sprites.setDataNumber(playerSprite, "vy", jumpVelocity)
    }
}
function updateObstacle (sprite: Sprite) {
    sprites.changeDataNumberBy(sprite, "x", obstacleSpeed * -1)
    sprite.setPosition(sprites.readDataNumber(sprite, "x"), 90)
    sprites.changeDataNumberBy(sprite, "i", 1)
    if (sprites.readDataNumber(sprite, "i") > sprites.readDataNumber(sprite, "iEnd")) {
        sprites.setDataNumber(sprite, "i", sprites.readDataNumber(sprite, "iStart"))
    }
    sprite.setImage(frames[sprites.readDataNumber(sprite, "i")])
    if (sprites.readDataNumber(sprite, "x") <= -16) {
        resetObstacle(sprite, getNextPosition())
    }
}
function resetObstacle (obstacle: Sprite, x: number) {
    sprites.setDataNumber(obstacle, "x", x)
    obstacle.setPosition(x, 90)
    sprites.setDataNumber(obstacle, "i", sprites.readDataNumber(obstacle, "iStart"))
}
function intialiseObstacleAnimation (obstacle: Sprite, idxStart: number, idxEnd: number) {
    sprites.setDataNumber(obstacle, "iStart", idxStart)
    sprites.setDataNumber(obstacle, "iEnd", idxEnd)
    sprites.setDataNumber(obstacle, "i", idxStart)
}
function initialiseScore () {
    scoreSprite = textsprite.create("0.0")
    scoreSprite.setOutline(1, 15)
    scoreSprite.setPosition(25, 16)
}
function initialiseAllObstacles () {
    obstacleSprite1 = sprites.create(frames[0], SpriteKind.Enemy)
    intialiseObstacleAnimation(obstacleSprite1, 0, 5)
    obstacleSprite2 = sprites.create(frames[6], SpriteKind.Enemy)
    intialiseObstacleAnimation(obstacleSprite2, 6, 8)
    obstacleSprite3 = sprites.create(frames[9], SpriteKind.Enemy)
    intialiseObstacleAnimation(obstacleSprite3, 9, 12)
    obstaclesList = [obstacleSprite1, obstacleSprite2, obstacleSprite3]
    resetAllObstacles()
}
function killPlayer () {
    if (!(died)) {
...
function resetAllObstacles () {
    for (let obstacle of obstaclesList) {
        sprites.setDataNumber(obstacle, "x", -16)
    }
    for (let obstacle of obstaclesList) {
        resetObstacle(obstacle, getNextPosition())
    }
}
function initialisePlayer () {
    playerSprite = sprites.create(frames[13], SpriteKind.Player)
    resetPlayer()
}
function updatePlayerVelocity () {
    if (sprites.readDataNumber(playerSprite, "y") < 0) {
        sprites.setDataNumber(playerSprite, "y", 0)
        sprites.setDataNumber(playerSprite, "vy", 0)
    } else {
        sprites.changeDataNumberBy(playerSprite, "vy", -1)
    }
    playerSprite.setPosition(16, 90 - sprites.readDataNumber(playerSprite, "y"))
}
function resetPlayer () {
    playerSprite.setPosition(16, 90)
    sprites.setDataNumber(playerSprite, "frame", 0)
    sprites.setDataNumber(playerSprite, "vy", 0)
    sprites.setDataNumber(playerSprite, "y", 0)
    died = false
...
function updateScore () {
    score += 1
    scoreText = convertToText(score / 10)
    if (!(scoreText.includes("."))) {
        scoreText = "" + scoreText + ".0"
    }
    scoreSprite.setText(scoreText)
}
function getNextPosition () {
    position = 0
    for (let obstacle of obstaclesList) {
        position = Math.max(position, sprites.readDataNumber(obstacle, "x"))
    }
    if (position > 160) {
        if (randint(0, 100) < 30) {
            position = position + randint(16, 17)
        } else {
            position = position + randint(50, 110)
        }
    } else {
        position = position + randint(50, 110)
    }
...
let position = 0
let scoreText = ""
let obstacleSprite3: Sprite = null
let obstacleSprite2: Sprite = null
let obstacleSprite1: Sprite = null
let died = false
let obstaclesList: Sprite[] = []
